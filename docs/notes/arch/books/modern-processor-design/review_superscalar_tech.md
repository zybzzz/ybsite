...
对于目的地址的预测，引入的是 BTB 这种 kv-cache，将分支指令的 pc 作为 key，将分支指令的预测目标地址作为 value。在取指阶段的时候，就能够通过访问 BTB 来得到预测的分支目标地址。分支指令第一次访问 BTB 的时候，BTB 可能还是空的，这就需要在本次分支完成之后进行填充。BTB 怎么去做功能上的实现应该依具体的算法不同而异，重要的是知道有这么个东西来实现对于分支目标地址的查找。另外非常重要的是这项技术发生的流水线阶段，根据图中也可以看到，访问 BTB 和访问 Icache 是同步进行的，因此这项技术的发生阶段在取指阶段，这样就保证了在下个时钟周期仍然能够通过前一步预测的地址来取指令，而不导致指令的断流，加入分支预测发生在译码阶段，就会造成指令的断流了。
...
- 保留站(reservation station,RS)：防止寄存器依赖不满足的时候对前序流水线产生的阻塞。
- 通用数据总线(common data bus,CDB)：在计算得到结果的时候快速前递数据。让保留站中的数据更快满足依赖条件。
- 寄存器重命名机制(Tag)：通过对 FLB/RS 进行 Tag 编号以及 CDB 的机制，让 FLB/RS 两者者联合起来像一个 RRF(重命名寄存器文件)。

在引入这些新的特性之后，一个重要的问题是，Tag 的含义， 谁是 Tag 的产生者，谁是 Tag 的消费者：

- Tag 的含义：Tag 为 0 时表示没有监听的对象，Tag 非 0 时代表监听对象。
- Tag 产生者：由于 FLB/RS 两者都能作为寄存器重命名的对象，因此 Tag 编码的地址范围要涵盖这两者，这两者的数量总共加起来有 11 个，编码这个 Tag 至少需要 4 位。SDB 不作为数据的产生者，这是因为数据都在被处理完成之后才向 SDB 去写，写入 SDB 的数据短期不会在被拿来使用。
- Tag 消费者：Tag 消费者主要包括保留站中的指令(10个)， TLR(4个) 和 SDB(3个)。

::: details 所谓的寄存器重命名
另一个角度对寄存器重命名的理解是重命名寄存器先一步于体系结构寄存器的更新。对应于这种情况，在 tomasulo 中，Tag 应当作为重命名寄存器，Tag 消费者作为实际的体系结构寄存器。实际上就是 Tag 一定程度上代替了体系结构寄存器，成为一些指令需要读取数据的来源。比如说我原来需要从 R1 读，现在从名称为 2 的 Tag 中读。
:::

自然的去想这个算法的工作过程：

1. 指令从 FLOS 中发出，进行译码。
   - 对于要读的寄存器，进行监测，看看能不能直接读到，能直接读到就直接读取，读不到就根据要读的寄存器记录Tag(RAW 相关)，后续进行监听。
   - 对于要写的寄存器或者说是内存地址，在指定 FLR 寄存器中的指定字段将 Tag 设置为即将**放置到保留站的 Tag**。
   - 对于 ld，直接发出。需要注意的是 FLB 正确取到数据的时候在 CDB 上广播自己的 Tag。
   - 对于 st，如果发射的时候依赖还没满足，就要在 SDB 的线管条目中记录 Tag，等待满足之后进行 st 的操作。
2. 指令很快发送到保留站中。
...
::: details 对于 WAW 依赖的消除？
其 ISA 设计本身就不会蕴含 WAW 的依赖，产生 WAW 的情况其实已经产生了 RAW。
...
在第一条指令发射的时候 r1 的 tag 被设置成保留站 1 的值。在第二条指令被发射的时候 r1 tag 被设置成保留站 2 的值。
