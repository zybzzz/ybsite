...
激活时候传入的 block 被调度到各个 SM 上，并且就在一个 SM 上被一直驻留，不会离开。同时 SM 安排的策略是有
套路的，不可控的，总体来说：
...
执行完 thread 后，完事一个释放相关的寄存器，然后补一个上来。就这样直到全做完为止。
一般来说 block 内的 thread 数为 256/512 等感觉比较固定，或许不是一个太值得深思的指标，
等研究着看吧。
